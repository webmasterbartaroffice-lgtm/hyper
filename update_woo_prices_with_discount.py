...
def load_excel_data(file_path):
    ...
    # Dictionary to store both regular price and sale price
    sku_price_data = {}
    
    for _, row in df.iterrows():
        sku = str(row['كد كالا']).strip() if pd.notna(row['كد كالا']) else None
        sale_price = float(row['قيمت فروش  ']) if pd.notna(row['قيمت فروش  ']) else None
        discount = float(row['مبلغ تخفيف']) if pd.notna(row['مبلغ تخفيف']) else 0
        
        if sku and sale_price is not None:  # اگر کد کالا و قیمت فروش معتبر باشند
            if discount > 0:  # اگر تخفیف داشته باشد
                regular_price = sale_price  # قیمت عادی = قیمت فروش
                final_sale_price = sale_price - discount  # قیمت نهایی = قیمت فروش - تخفیف
            else:
            ...
            sku_price_data[sku] = {
                ...
                'sale_price': final_sale_price if discount > 0 else None
            }
            ...
            "update": [
                {
                    ...
                    **({"sale_price": str(price_data['sale_price'])} if price_data['sale_price'] is not None else {})
                }
...
def main():
    try:
        ...
        # Create a mapping of SKU to product ID and current prices
        sku_to_product = {
            product['sku']: {
                'id': product['id'],
                'current_regular_price': product['regular_price'],
        ...
        for sku in matching_skus:
            new_prices = sku_price_data[sku]
            current_regular = float(sku_to_product[sku]['current_regular_price']) if sku_to_product[sku]['current_regular_price'] else 0
            current_sale = float(sku_to_product[sku]['current_sale_price']) if sku_to_product[sku]['current_sale_price'] else None
            
            if (current_regular != new_prices['regular_price'] or 
                current_sale != new_prices['sale_price']):
                updates_needed.append((
                    sku_to_product[sku]['id'],
                    new_prices
                ))
                print(f"\nWill update {sku}:")
                print(f"Regular Price: {current_regular} → {new_prices['regular_price']}")
                if new_prices['sale_price'] is not None:
                    print(f"Sale Price: {current_sale} → {new_prices['sale_price']}")
